...
> 状态：原理图与 PCB PDF 快照已归档；历史 DRC 结论不能替代当前制造复核
...
**硬件事实以仓库 `hardware/` 下 2026-09-18 原理图/PCB PDF 快照为准。** 本说明是设计的同步记录；PDF 不能证明未印出的实时布线统计、制造状态或整机行为。`PCB1.epro2` 已删除，MC100 不保留或上传嘉立创/EasyEDA 工程。
...
本次逐页检查五页原理图和七页 PCB PDF。历史评审曾记录 56 个放置项、234 个焊盘和 DRC 结果，但原始实时读取文件已不在仓库，不能从本仓库复现；这些数字不替代 PDF 或制造放行检查。

历史评审对制造文件的观察保留为待核验风险，不等同于当前 PDF 已证明的制造结论。原生 USB、GPIO0 和 EN 的连接以 PDF 页图为准，首次烧录和强制恢复的步骤见 [MC100-PROGRAMMING.md](MC100-PROGRAMMING.md)。
...
| 存储 | SDMMC 1-bit microSD，当前 EVT 使用 64 GB exFAT 卡，仍需多卡型验证 | 比 SDSPI 更符合 S3 能力，三根总线信号已远超 32 kB/s 音频需求 |
...
- 当前 EVT 已验证一张 64 GB exFAT microSD；产品支持范围仍需至少三种卡型的实测收敛。
- 量产前至少测试 3 个具体卡型号。
...
PDF 快照不印出可复现的 segments/arcs/vias/pours/fills 统计；历史实时读取中的几何数字因此降级为不可复核，不在本说明中作为当前事实。R22 源端阻尼位置、C20 卡座就近去耦、丝印和阻抗叠层仍需独立制造复核，详见 [PCB 复核报告](MC100-PCB-REVIEW.md)。

### 11.2 结构位置

下表为历史评审记录中的器件 bbox 几何中心，单位 mil；原始实时工程不在仓库，不能把这些坐标当作当前制造依据。
...
以下依据 2026-09-08 历史器件记录整理，共 52 个有采购料号的器件、25 种不同料号。SMT 服务分类和库存价格会变化，以实际下单时为准，批次与替代料尚未核对；这些记录不表示仓库保留或上传了 EDA 工程。
...
### 12.4 SMT 经济型/标准型历史核对（2026-09-08）

**历史 BOM 记录中，U3 / C2913206 和 U4 / C42372687 被器件库标记为“标准型SMT专用”。** 其余 23 种料号、50 颗元件当时未显示该限制；当前服务资格必须由实际代工方重新确认。
...
| PCB 未布线、需重新建电源铜、DRC 9 项 | 当前已布线，官方 DRC 0 项；具体直线段/圆弧/过孔/铺铜区数量见 §11.1，PDF 快照不含布线统计，不作为当前权威事实 |
...
旧的 63/68 项布局、预布线审计和缺货替换时的违规列表已由本次基线取代。后续变更以新导出的 PDF 快照为准，不把历史位号或未布线状态恢复到工程。
...
2026-09-18 的硬件现状以仓库 PDF 快照的器件表、页图和已记录的文字复核为准。器件位号、Value 和 GPIO 接线不得从缺失的实时工程推断。
...
- 历史物料编号记录来自 2026-09-08 的评审记录；本次不作库存或价格时效性承诺
