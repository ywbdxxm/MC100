...
and speech are preserved. Multiply the resulting sample by the configured
integer gain, then clamp to signed 16-bit range. Count samples that clamp and
record input/output peak magnitudes. Use 64-bit intermediates throughout.
...
Host tests cover constant DC, bypass identity, gain, clipping, invalid gain,
and continuity across frame boundaries. Build the product firmware with the
