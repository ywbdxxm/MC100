...
| T09 IDF 外设适配 | EVT_IN_PROGRESS | 3秒与60秒采集通过；64GB exFAT挂卡、3秒WAV落盘及完整CRC读回通过，长录音验证中 |
| T10 监控/配置/诊断 | NOT_STARTED | 低电参数保留 EVT 标识 |
| T11 任务集成/资源 | EVT_SLICE | 单独Audio/Storage所有者，手动USB台架入口；完整自动State/VAD运行未完成 |
...
2026-09-19最新统一测试18/18通过；14套C核心测试分别运行ASan/UBSan，共28/28通过（后加capture/sync以新增原生回归覆盖）。目标固件按固定ESP-IDF构建和分区检查通过。原8MB Flash完整备份后才写入台架固件，芯片写入哈希校验通过。实际采集、卡格式与尚未完成项目见[实板报告](reports/2026-09-19-evt-recording.md)。
