...
| `product` | `main/app_main.c` → `record_loop.c` → PDM/PCM 处理/有界队列/writer | 上电自动录音，默认 20 秒，通常生成一对 WAV/IDX 后进入 IDLE | product 目标构建和镜像写入校验已通过；当前板子停在 ROM DOWNLOAD，COM7/SD 实板验证待释放 BOOT 后重试 |
